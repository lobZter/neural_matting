...
# DATA_PATH = os.path.join(PROJ_ROOT, "dataset", "npy")
DATA_PATH = "npy"
...
img_normalized = tf.map_fn(lambda x: pre_process_image(x), img)
X_preprocessed = tf.concat([img_normalized, alphas], axis=3)
...
# loss
predict_alpha = conv6
tf.summary.image('input_img',     img[:1,:,:,:],           max_outputs=1)
tf.summary.image('predict_alpha', predict_alpha[:1,:,:,:], max_outputs=1)
tf.summary.image('ground_true',   Y_resized[:1,:,:,:],     max_outputs=1)
